...
static int create_table(appdata_s *ad){
	char err_msg;
	char *sql = "CREATE TABLE IF NOT EXISTS Recipes(KEY INTEGER PRIMARY KEY, "
			"NAME TEXT NOT NULL,"
...
//Слушатель, вызывающийся для каждой строки, считанной из бд
static int db_read_cb(void *counter, int argc, char **argv, char **azColName){
	...
	char name[255];
	strcpy(name, argv[1]);
	elm_list_item_append(m_ad->list, name, NULL, NULL,
			recipes_detail_cb, n_key);
	elm_list_go(m_ad->list);
	return 0;
}

//Чтение всех рецептов из бд
static int read_db(appdata_s *ad){
	char *sql = "select * from Recipes";
	int counter = 0;
	char *err_msg;
...
static void init_db(appdata_s *ad){
	...
	int size = strlen(resource) + 10;
	char *path = malloc(sizeof(char)*size);
	strncat(path, resource, size);
	strncat(path, "recipes.db", size);
